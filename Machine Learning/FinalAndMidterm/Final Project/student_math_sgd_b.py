...
def nn(iter, _x, _y, alpha, inputSize, featureSize):
    ...
    wn = np.ones((featureSize, featureSize)) * .1
    b1 = np.ones((featureSize, 1))
    b2 = np.ones((featureSize, 1))
    bn = np.ones((1, 1))
    theta = np.ones((featureSize + 1, 1))
    loss_list = []
    for k in range(iter):
        # Forward
        z1 = (w1 @ _x) + b1
...
def nnPredict(_x, w1, b1, w2, b2, wn, bn):
    z1 = (w1 @ _x) + b1
    a1 = relu(z1)
...
w1, b1, w2, b2, wn, bn, loss = nn(1000, X_train.T, y_train, .05, X_train.shape[0], X_train.shape[1])
result = nnPredict(X_test.T, w1, b1, w2, b2, wn, bn)
print(loss)
print(y_test.shape)
print(result.shape)
