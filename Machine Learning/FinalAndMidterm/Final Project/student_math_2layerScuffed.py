...
def nn(iter, featureN, _x, _y, alpha):
    ...
    for k in range(iter):
        ...
        for z in range(curX.shape[0]):
            ...
            weightMatrixLast[z] -= alpha * (z2prime *  np.sum(weightMatrixLast[0]))[0]
...
print('start')
# w1, w2, loss = nn(2000, len(X_train.T), X_train, y_train, .0005)
w1, w2, loss = nn(2500, len(X_train.T), X_train, y_train, .0005)
